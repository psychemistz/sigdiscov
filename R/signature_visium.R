...
#' @param spot_coords A data frame or matrix with spot coordinates. For
...
#'   grid coordinates.
#' @param weight_type Weight computation method: "circular" (default, matches
#'   SpaCET) or "grid" (legacy hexagonal grid-based).
#' @param radius For circular weights: distance cutoff in physical units
#'   (default: 200). For grid weights: converted to max_radius.
#' @param sigma RBF kernel bandwidth for circular weights. Default: 100.
#' @param max_radius Maximum grid radius for grid-based weights. Default: 5.
#'   Ignored when weight_type="circular".
#' @param platform Platform type for grid weights: "visium" or "old".
#'   Ignored when weight_type="circular".
#' @param same_spot Whether to include self-connections. Default: FALSE.
...
pairwise_moran <- function(data,
    ...
    # Use circular weights (SpaCET-compatible, default)
    if (weight_type == "circular") {
        result <- pairwise_moran_circular(
            data = data,
            coords = spot_coords,
            radius = radius,
            sigma = sigma,
            include_self = same_spot,
            mode = mode,
            verbose = verbose
        )
        return(result)
    ...
    # Return in consistent format with circular
    return(list(
        moran = result,
        gene_names = gene_names,
        weight_sum = NA  # Grid method doesn't easily expose this
    ))
}
