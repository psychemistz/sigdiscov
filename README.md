...
devtools::install_github("psychemistz/sigdiscov@v1.1.0", subdir = "sigdiscov")
...
Distance-dependent spatial correlation curves to distinguish responsive genes from constitutively expressed genes.
...
# Get delta I matrix for all gene pairs
delta_matrix <- compute_delta_I_matrix(
    expr_matrix = data_vst,
    coords = coords,
    radii = seq(50, 300, 50)
)
...
### 3. Four Delta Metrics
Compute comprehensive spatial signatures with different weight types.

```r
# Compute four delta metrics for a factor
deltas <- compute_four_deltas(
    expr_matrix = data_vst,
    coords = coords,
    factor_gene = "IL1B",
    radii = seq(50, 300, 50)
...
# Returns: delta_ring_moran, delta_ring_ind, delta_circ_moran, delta_circ_ind
```
