...
int main(int argc, char *argv[])
{
	...
	// Whether calculate paired genes or single gene
	bool pariedGenes_singleGene = true;
	
	// Whether calculate all genes or only 1st gene
	...
	if ( argc < 5 )
	{
		if(argc == 2 && (value = argv[1]) == "-h")
		{
			...
			cout << "\t-b\t\tWhether calculate paired genes (1) or single gene (0). Default: " << pariedGenes_singleGene << endl;
	...
	for(i=0;i<parseCnt;i++)
	{
		...
		if(type == "-i"){
			...
			pariedGenes_singleGene = (value[0]!='0');
