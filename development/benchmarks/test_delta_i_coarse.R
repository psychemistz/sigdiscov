#!/usr/bin/env Rscript
# Delta I computation with coarser spacing (100 um steps)
# 960 genes x 98k cells, 50 radii (100-5000 um)

library(sigdiscov)
library(rhdf5)

cat("=== Delta I Computation (100 um spacing) ===\n")
...
# Create HDF5 file
h5_file <- "output/moran_curves_100um.h5"
...
for (i in seq_along(radii_mm)) {
    ...
    # Progress every radius
    avg_neighbors <- result$n_edges / n_cells
    eta <- (total_time / i) * (n_radii - i) / 60
    cat(sprintf("  Radius %2d/%d (%4d um): %6.1f sec, %5.0f neighbors/cell, ETA: %5.1f min\n",
                i, n_radii, radii_um[i], elapsed, avg_neighbors, eta))
    flush(stdout())
}

H5close()
cat(sprintf("\nTotal Moran computation: %.1f min (%.1f hours)\n",
...
saveRDS(list(
    ...
    n_cells = n_cells
), "output/delta_I_100um_results.rds")
...
cat(sprintf("\nTotal time: %.1f min (%.1f hours)\n", total_elapsed, as.numeric(total_elapsed)/60))
cat(sprintf("HDF5 file: %s\n", h5_file))
cat("Results saved to: output/delta_I_100um_results.rds\n")
