#!/usr/bin/env Rscript
# Full Delta I computation with streaming only + HDF5
# 960 genes x 98k cells, 250 radii (20-5000 um)

library(sigdiscov)
library(rhdf5)

cat("=== Full Delta I Computation (Streaming + HDF5) ===\n")
...
# Compute and stream Moran matrices (streaming only)
...
for (i in seq_along(radii_mm)) {
    ...
    # Progress every 5 radii
    if (i %% 5 == 0 || i == 1 || i == n_radii) {
