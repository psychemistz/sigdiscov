#!/usr/bin/env Rscript
# COAD TGFB1 Analysis with GLOBAL normalization (optimized for large sparse matrix)

library(sigdiscov)
library(Matrix)
library(rhdf5)

cat("=== COAD TGFB1 Analysis (h5ad VST, Global Norm, Optimized) ===\n\n")
...
# =============================================================================
# 2. Define Sender and Receiver FIRST (before computing global stats)
# =============================================================================
...
# =============================================================================
# 3. Compute GLOBAL statistics (efficiently for sparse matrix)
# =============================================================================

cat("\n--- Computing Global Statistics (Optimized) ---\n")

# For sparse matrix, compute mean efficiently
global_means <- Matrix::rowMeans(expr)

# Compute variance using: var = E[X^2] - E[X]^2
# For sparse matrix, squaring creates lots of nonzeros - compute by chunks
cat("Computing global variance (chunked)...\n")
chunk_size <- 10000
n_chunks <- ceiling(n_cells / chunk_size)

global_sum_sq <- numeric(n_genes)
for (i in seq_len(n_chunks)) {
    start_idx <- (i - 1) * chunk_size + 1
    end_idx <- min(i * chunk_size, n_cells)
    chunk <- as.matrix(expr[, start_idx:end_idx])
    global_sum_sq <- global_sum_sq + rowSums(chunk^2)
    if (i %% 5 == 0) cat(sprintf("  Chunk %d/%d\n", i, n_chunks))
}
global_mean_sq <- global_sum_sq / n_cells
global_vars <- global_mean_sq - global_means^2
...
# Receiver expression - keep as matrix for faster lag computation
cat("Extracting receiver expression...\n")
receiver_expr <- as.matrix(expr[, receiver_idx])
...
for (r in seq_along(radii)) {
    ...
    cat(sprintf("  Radius %d um (ring %d-%d)...", radius, inner_radius, radius))
    ...
    # Compute spatial lag (raw): W %*% receiver_expr^T = n_senders x n_genes
    lag_G_raw <- as.matrix(W %*% t(receiver_expr))
